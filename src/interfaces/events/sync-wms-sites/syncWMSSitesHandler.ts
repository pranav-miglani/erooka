/**
 * WMS Site Sync Lambda Handler
 * 
 * EventBridge trigger: Runs daily
 * Syncs WMS sites from all active WMS vendors.
 */
...
export async function handler(
  ...
  console.log("[WMSSiteSync] Event received:", JSON.stringify(event, null, 2))

  try {
    const results = await wmsSyncService.syncAllVendorSites()

    console.log("[WMSSiteSync] Sync completed:", JSON.stringify(results, null, 2))
    ...
    console.error("[WMSSiteSync] Error:", error)
    throw error
  }
}
